...
.venv/bin/python -m pytest                          # full suite (163 tests)
...
Coalescing state (`PENDING_CHANGES`, `PENDING_SINCE`, `LAST_CHANGE_AT`, `LAST_APPLY_AT`) is module level and measured with `time.monotonic()`, so a wall-clock adjustment cannot strand pending changes. All four start as `None`, and `should_apply_immediately()` special-cases that rather than doing arithmetic on it — so an unset `LAST_APPLY_AT` reads as *long ago*, not *just now*. A test that needs "an apply just happened" must set `time.monotonic()`.
